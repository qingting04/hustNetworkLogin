...
#   5. SIGTERM 干净退出：state=stopped、删 pidfile、对象从总线注销
#
# 用法（在仓库根目录）：
...
state_field() { sed -n "s/^$1=//p" "$RUN/hust-network-login.state" 2>/dev/null; }
...
mkdir -p "$RUN"
rm -f "$RUN/hust-network-login.state" "$RUN/hust-network-login.pid"
...
[ "$(state_field ubus)" = "unavailable" ]
check $? "ubusd 不在时状态文件写 ubus=unavailable（state=$(state_field state)）"
...
[ "$(state_field ubus)" = "registered" ]; check $? "状态文件写 ubus=registered"
...
[ "$(state_field ubus)" = "registered" ]; check $? "状态文件重新写回 ubus=registered"
...
[ $? -ne 0 ]; check $? "进程已退出"
[ "$(state_field state)" = "stopped" ]; check $? "退出时写 state=stopped"
